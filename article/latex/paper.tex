...
GRU & L2 & 10.06M & 3 & 980M & $0.5465 \pm 0.0768$ \\
GRU & L3 & 10.25M & 3 & 990M & $0.3241 \pm 0.0862$ \\
\midrule
...
MoSAIC & L2C16 & 10.09M & 3 & 1000M & $0.8911 \pm 0.0049$ \\
...
\paragraph{Results.}
Table~\ref{tab:sdq} shows a consistent family-level advantage for MoSAIC: every evaluated shape has a higher central \texttt{Acc++} value than every GRU depth. In the replicated two-layer comparison, L2C4 reaches $0.8433 \pm 0.0055$, while GRU-L2 reaches $0.5465 \pm 0.0768$. L2C16 reaches $0.8911 \pm 0.0049$, and L3C4 is the best observed configuration at $0.9204 \pm 0.0128$.

\begin{figure*}[t]
\centering
\includegraphics[width=0.96\textwidth]{fig_learning_curves.pdf}
\caption{Learning dynamics under the standard 1B-token protocol. Lines show the mean across completed replicates and shading shows $\pm 1$ standard deviation. All displayed text8 and SDQ curves use three replicates. (a) Validation BPC on text8. (b) Online long-gap query accuracy on SDQ. Final SDQ values in Table~\ref{tab:sdq} use the mean of each replicate's last five logged values.}
...
HGRN2, DeltaNet, and mLSTM exceeded available memory at the standard 512-stream geometry and were run separately under non-comparable token, batch, and update budgets. We draw no direct quality or efficiency comparison from them.
