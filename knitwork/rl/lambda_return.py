...
from knitwork.rl.flags import RlFlags


class LambdaReturn:
    """
    Class for TD(lambda) returns calculation.
    To compute GAE, you have to subtract V[t], or in vectorised form
        GAE = lambda_ret - V
    ...
    def __call__(self, V, r, flags, G, t):
        ...
        unsqueeze = flags[0].ndim == r[0].ndim < V[0].ndim
        ...
        while t > 0:
            ...
            r_t = r[t].unsqueeze(-1) if unsqueeze else r[t]
            flags_tn = flags[t].unsqueeze(-1) if unsqueeze else flags[t]
            term_tn = flags_tn & (RlFlags.TERMINATED | RlFlags.RESET)
            trunc_tn = flags_tn & RlFlags.TRUNCATED
            ...
            G_t = r_t + gamma * backup
            rev_G.append(G_t)
            assert G_t.shape == V_tn.shape == r_t.shape
