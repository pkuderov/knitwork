...
class LRUCell(nn.Module):
    def __init__(self, d_in, d_state, d_out):
        super().__init__()

        self.log_r = nn.Parameter(...)
        self.theta = nn.Parameter(...)

        self.B = nn.Linear(d_in, d_state, dtype=torch.cfloat)
        self.C = nn.Linear(d_state, d_out, dtype=torch.cfloat)

    def init_radii(self):
        phase = 2 * pi * rand(d_state)
        
        # choose |lambda| in some annulus near the unit circle
        r2 = uniform(r_min**2, r_max**2)
        r = sqrt(r2)

        # inverse of r = exp(-exp(log_r))
        log_r = log(-log(r))

        theta = phase

    def forward(self, x, h):
        lam = torch.exp(-torch.exp(self.log_r) + 1j * self.theta)
        gamma = torch.sqrt(1 - lam.abs() ** 2)

        h = lam * h + gamma * self.B(x.to(torch.cfloat))
        y = self.C(h).real
