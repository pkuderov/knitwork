# run_sdq2.py ─ SDQ-тренировка с поддержкой GridRnn2 (VAE + TimeGate + ColDropout)
...
def main(config):
    ...
    # ── Флаг: является ли модель GridRnn2 ────────────────────────────────
    # GridRnn2.forward возвращает (y, h, kl_loss) вместо (y, h)
    # VAE считается активным, если vae_latent_dim задан и не None в конфиге
    is_grnn2  = rnn_type == 'grnn2'
    vae_active = is_grnn2 and (rnn_cfg.get('vae_latent_dim') is not None)
    print(
        f'[VAE] {"АКТИВЕН" if vae_active else "ВЫКЛЮЧЕН"} '
        f'(is_grnn2={is_grnn2})'
    )
    ...
    # ── KL Annealing (только для grnn2 с активным VAE) ───────────────────
    #
    # Простыми словами:
    #   В начале обучения KL-штраф = 0, сеть просто учится предсказывать.
    #   Постепенно вес KL растёт до kl_anneal.max_weight.
    #   Это предотвращает «posterior collapse» — явление, когда VAE
    #   игнорирует латентную переменную и сводит KL к нулю с самого начала.
    #
    # Управление из конфига:
    #   kl_anneal:
    #     steps: 100_000      # шагов до достижения max_weight (0 = мгновенно)
    #     max_weight: 1.0     # максимальный итоговый множитель
    ...
    def get_kl_anneal(current_step: int) -> float:
        """Линейно растёт от 0 до kl_anneal_max за kl_anneal_steps шагов."""
        if not vae_active:
            return 0.0
    ...
    batch_y_gt    = []
    batch_sq_gaps = []
    batch_kl      = []   # KL-лоссы за rollout (только grnn2 + vae)
    ...
    while step < n_steps:
        ...
        # ── Forward pass ─────────────────────────────────────────────────
        # Режим capture: за один шаг до порога включаем return_attn=True
        # чтобы собрать веса внимания и gate-статистику для визуализаций.
        capture = (step >= next_vis_step - gen.n_envs)

        if capture:
            if is_grnn2:
                # grnn2 с return_attn=True → (y, h, extras, kl_loss)
                y, rnn_state, extras, kl_loss = rnn(x, rnn_state, return_attn=True)
            else:
                # старые модели → (y, h, extras)
            ...
            # Средние значения gate-ов по слоям для логирования
            gate_vals = [
            ...
            if is_grnn2:
                # grnn2 без визуализации → (y, h, kl_loss)
        ...
        if step % batch_size == 0:
            ...
            # KL-лосс: среднее за rollout × линейный annealing коэффициент
            # Если VAE выключен — kl_mean = 0, total_loss = ce_loss
            ...
            # Метрики по sq_gaps (идентично оригиналу)
            ...
            # KL-метрики добавляются только если VAE активен
            if vae_active:
        ...
        if print_stats_schedule.tick(gen.n_envs):
            ...
            # KL строка — только если VAE включён
            kl_str = (
                f' KL: {metrics.get("KL", 0):.2e}'
                f' (x{metrics.get("KL_scale", 0):.2f}) |'
                if vae_active else ''
            )
            print(
