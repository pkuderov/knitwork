...
|---|---|---|---|---|---|
| RepeatFirstEasy | Object | ~−0.4…−0.5 | 1.07 | 338 | не обучается, остановлено |
| RepeatPreviousEasy | Object + Sequential | −0.246 | 0.64 | 337 | коллапс, остановлено |

**RepeatPreviousEasy** — пик EpRet=0.93 к 3M шагов, затем коллапс энтропии до H=0.19 и деградация. К 15M шагов H=0.64, EpRet продолжает падать — коллапс необратим. Причина: LRU без явного entropy regularization быстро детерминизируется при первом успехе. 

**RepeatFirstEasy** — EpRet осциллирует в диапазоне −0.5…−0.4 без тенденции. H=1.07 (здоровое исследование), но без прогресса за 7.5% обучения. LRU-рекуррентность недостаточна для Object memory без cross-column attention (в отличие от hgrnn_lru).

Оба запуска остановлены досрочно для экономии ресурсов.
