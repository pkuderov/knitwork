...
### MIKASA / POPGym (предварительно, 22M/200M шагов)

| Среда | Тип памяти | EpRet | Прогресс | FPS | Вывод |
|---|---|---|---|---|---|
| RepeatFirstEasy | Object | **-0.48** | 22M/200M (11%) | 586 | Не обучается |
| HigherLowerEasy | Sequential | — | pending | — | — |

**RepeatFirstEasy** — на 22M шагов (11% обучения, LR уже на 98%) EpRet отрицательный: агент выбирает действия хуже случайного. Модель существенно медленнее: 586 fps против ~1000 fps у grnn_ema_mem — двухмасштабная delta-память значительно увеличивает вычислительную нагрузку (~1.7× per step). Возможные причины неудачи: (1) избыточная ёмкость модели для простой Object-задачи; (2) delta-правило хорошо справляется с точным ключ-значение поиском, но для RL-задач с разреженным и шумным обучающим сигналом требует больше шагов прогрева; (3) медленная скорость обучения из-за высокого FLOPs per step ограничивает реальное число обновлений за единицу времени.

> Результаты предварительные — 11% обучения. Финальный вывод после полного прогона.
